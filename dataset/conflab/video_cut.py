...
read_path = '/home/zonghuan/tudelft/projects/datasets/modification/conflab_temp'
IMG_PER_VIDEO = 8
SEGMENT_PER_VIDEO = 2
SEGMENT_DURATION = 5
...
def select_random_frames_dataset(d_path, v_paths, n):
    ...
    for v_path in v_paths:
        ...
        all_frames.append(select_frames(v_path_complete, n))
    return all_frames

def select_frames(v_path, n):
    # Open the video file
    ...
    frame_indices = np.linspace(start_frame, end_frame, n).round().astype(int).tolist()
    selected_frames = []
...
def conflab_video_paths():
    return [x for x in os.listdir(read_path) if x.endswith(('.MP4', '.avi', '.mov', '.mkv'))]
...
def video_to_img_selection(n_frames):
    ...
    selected_frames = select_random_frames_dataset(read_path, video_paths, n_frames)
...
def single_video_clip(video_path, segment_duration, write_path):
    ...
    frame_indices = np.linspace(int(0.4 * video.duration), video.duration - 5, SEGMENT_PER_VIDEO).round().astype(int).tolist()
