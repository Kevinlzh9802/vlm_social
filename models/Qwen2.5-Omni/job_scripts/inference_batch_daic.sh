#!/bin/bash
#SBATCH --job-name="qwen_inference_daic"
#SBATCH --time=2:00:00
...
#   sbatch job_scripts/inference_batch.sh --dataset mintrec2 --mode context --batch 2 -model 7B -prompt intention --annotated
#   sbatch job_scripts/inference_batch.sh --dataset mintrec2 --mode context --batch 2 -model 7B -prompt intention --annotated --comparison
...
usage() {
    ...
    echo "       sbatch job_scripts/inference_batch.sh --dataset <dataset> [--mode <context|nested>] --batch <number> [--conversation-mode <single-turn|multi-turn>] [-model 7B|3B] -prompt <prompt_choice> --annotated [--comparison] [--no-audio]" >&2
}
...
if [ "$annotated" = false ] && [ -z "$utt_count" ]; then
    usage
    exit 1
fi
...
if [ "$annotated" = true ]; then
    if [ "$comparison" = true ]; then
        ...
        if [ "$no_audio" = true ]; then
            output_base_root="${gestalt_root}/human_eval/task2/manipulation_full/results_noaudio_comparison/qwen2.5"
        else
            output_base_root="${gestalt_root}/human_eval/task2/manipulation_full/results_comparison/qwen2.5"
        fi
    else
        data_base_root="${gestalt_root}/human_eval/task2/manipulation_full/data"
        if [ "$no_audio" = true ]; then
            output_base_root="${gestalt_root}/human_eval/task2/manipulation_full/results_noaudio/qwen2.5"
        else
            output_base_root="${gestalt_root}/human_eval/task2/manipulation_full/results/qwen2.5"
        fi
    fi
fi
